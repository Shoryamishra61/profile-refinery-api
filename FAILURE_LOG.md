...
| F-013 | Production returned `SYNTHETIC-001` for real profile URLs | Vercel was explicitly configured with `APP_MODE=fixture`; fixture transport intentionally ignored the requested slug | Added retrieval provenance, live-mode operation filtering, explicit 503 readiness/errors, and live fixture-sentinel regression guards; production now runs fail-closed live mode | FIXED + production verified |
| F-014 | Vercel demo embedded an API key and described fixture output as live | Uncommitted demo code treated a server secret as a browser credential and used misleading labels | Removed the embedded key, require caller entry, corrected authentication/evidence language, and hide the fixture demo in live mode | FIXED + tested |
